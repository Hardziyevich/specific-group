...
- Use Exceptions rather than returning code: Don’t return error flag or error code, instead always work with exceptions 
  if programming language allows.
- Try-Catch-Finally Statements: Use try-catch statements in all sections that you think it might not go well. 
  Finally, scope should be used if you are not sure whether catch scope will handle the situation correctly.
- Provide context: Give detailed information about exception but stay minimalistic.
